...
try
        {
            ...
            try
            {
                String qu="select * from subcategory where subcatid=?";
                PreparedStatement myst=myconnection.prepareStatement(qu);
                myst.setString(1, request.getParameter("subcatid"));
               ResultSet res=myst.executeQuery();
                if(res.next())
                {
                    cid=res.getString("catid");
    ...
    try
    {
        ...
        try
        {
            ...
            if(res.next())
            {
                do
                {
                    if(cid.equals(res.getString("catid")))
                    {
...
%>
					...
					<input type="text" placeholder="Sub Category" required=" " name="subcat" value="<%
                                               out.print(subname);
                                               %>">
...
<%
    if(request.getParameter("s1")!=null)
    {
        try
        {
            ...
            try
            {
                String qu="update subcategory set catid=?,name=? where subcatid=?";
                PreparedStatement myst=myconnection.prepareStatement(qu);
                myst.setString(1, request.getParameter("cat"));
                myst.setString(2, request.getParameter("subcat"));
                myst.setString(3, request.getParameter("subcatid"));
                int i=myst.executeUpdate();
                if(i==1)
                {
                    response.sendRedirect("msubcat.jsp?catid="+request.getParameter("cat"));
                }
                else
                {
                    out.print("problem");
